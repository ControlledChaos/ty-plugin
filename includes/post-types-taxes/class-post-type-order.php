...
if ( typ_acf_options() ) {
    $sort_order = get_field( 'typ_use_custom_sort_order', 'option' );

// If Advanced Custom Fields is not active.
} else {
    $sort_order = get_option( 'typ_use_custom_sort_order' );
}
...
class Post_Types_Taxes_Order {
	...
	public static function instance() {
		...
		if ( is_null( $instance ) ) {
            ...
            if ( ! get_option( 'typ_order_install ' ) ) {
    ...
    public function order_install() {
        ...
        update_option( 'typ_order_install', 1 );
    ...
    public function admin_menu() {
        ...
        add_options_page(
            __( 'Posts & Taxonomies Sort Order', 'ty-plugin' ),
            __( 'Sort Order', 'ty-plugin' ),
            'manage_options',
            'sort-order-settings',
            [ $this, 'admin_page' ]
        );
    ...
    public function admin_page() {
        ...
        require TYP_PATH . 'admin/partials/settings-page-posts-order.php';
    ...
    public function load_script_css() {
        ...
        if ( $this->_check_load_script_css() ) {
            ...
            wp_enqueue_script( 'post-tax-order', TYP_URL . 'admin/assets/js/post-tax-order.js', [ 'jquery' ], null, true );
    ...
    public function update_options() {
        ...
        if ( ! isset( $_POST['typ_posts_order_submit'] ) ) {
            return false;
        }

        check_admin_referer( 'typ_posts_order_nonce' );

        $input_options            = [];
        $input_options['objects'] = isset( $_POST['objects'] ) ? $_POST['objects'] : '';
        $input_options['tags']    = isset( $_POST['tags'] ) ? $_POST['tags'] : '';

        update_option( 'typ_order_options', $input_options );
    ...
    public function get_order_options_objects() {
        ...
        if ( $typ_order_options = get_option( 'typ_order_options' ) ) {
            $typ_order_options = get_option( 'typ_order_options' );
        } else {
            $typ_order_options = [];
        }

        if ( isset( $typ_order_options['objects'] ) && is_array( $typ_order_options['objects'] ) ) {
            $objects = $typ_order_options['objects'];
        } else {
            $objects = [];
        }
    ...
    public function get_order_options_tags() {
        ...
        if ( isset( $typ_order_options['tags'] ) && is_array( $typ_order_options['tags'] ) ) {
            $tags = $typ_order_options['tags'];
        } else {
            $tags = [];
        }
 ...
 */
function typ_post_types_taxes_order() {
...
function uninstall_db() {
    ...
    delete_option( 'typ_order_install' );
