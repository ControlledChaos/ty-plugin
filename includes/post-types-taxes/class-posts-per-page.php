 ...
 *
 * @since      1.0.0
 * @author     Greg Sweet <greg@typzine.com>
...
class Posts_Per_Page {
	...
	public function activate() {
		...
		$current_options   = get_option( 'typ_ppp_options' );
		$default_options   = [];
		...
		foreach ( $option_type_array as $option_type ) {
			...
			} elseif ( ! empty( $typ_ppp_options[ $option_type . '_count' ] ) ) {
		...
		update_option( 'typ_ppp_options', $default_options );
	...
	public function add_settings() {
		add_options_page( __( 'Posts Per Page', 'ty-plugin' ), __( 'Posts Per Page', 'ty-plugin' ), 'manage_options', 'posts-per-page', [ $this, 'view_settings' ] );
	}
	...
	public function view_settings() {
		...
		<div class="wrap posts-per-page">
			...
			<h1><?php _e( 'Posts Per Page', 'ty-plugin' ); ?></h1>
			<h2><?php _e( 'Overview', 'ty-plugin' ); ?></h2>
			...
			can be set for any other pages not covered by this.', 'ty-plugin' ); ?></p>
			<p><?php _e( 'The initial value used on activation was pulled from the setting <em>Blog Pages show at most</em> found in the', 'ty-plugin' ); ?> <a href="<?php echo site_url( '/wp-admin/options-reading.php' ); ?>" title="Reading Settings"><?php _e( 'Reading Settings', 'ty-plugin' ); ?></a></p>
			<form method="post" action="options.php">
				<?php
					settings_fields( 'typ_ppp_options' );
					do_settings_sections( 'typ_ppp' );
					do_settings_sections( 'typ_ppp_custom' );
				?>
			<p class="submit"><input type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'ty-plugin' ); ?>" /></p></form>
		</div>
		<?php
	}
	...
	public function register_settings() {
		...
		register_setting( 'typ_ppp_options', 'typ_ppp_options', [ $this, 'validate_options'] );

		add_settings_section( 'typ_ppp_section_main', '', [ $this, 'output_main_section_text' ], 'typ_ppp' );
		add_settings_section( 'typ_ppp_section_custom', '', [ $this, 'output_custom_section_text' ], 'typ_ppp_custom' );

		add_settings_field( 'typ_ppp_index_count',     __( 'Main Index posts per page', 'ty-plugin' ), [ $this, 'output_index_count_text' ],    'typ_ppp', 'typ_ppp_section_main' );
		add_settings_field( 'typ_ppp_category_count',  __( 'Category posts per page', 'ty-plugin' ),   [ $this, 'output_category_count_text' ], 'typ_ppp', 'typ_ppp_section_main' );
		add_settings_field( 'typ_ppp_archive_count',   __( 'Archive posts per page', 'ty-plugin' ),    [ $this, 'output_archive_count_text' ],  'typ_ppp', 'typ_ppp_section_main' );
		add_settings_field( 'typ_ppp_tag_count',       __( 'Tag posts per page', 'ty-plugin' ),        [ $this, 'output_tag_count_text' ],      'typ_ppp', 'typ_ppp_section_main' );
		add_settings_field( 'typ_ppp_author_count',    __( 'Author posts per page', 'ty-plugin' ),     [ $this, 'output_author_count_text' ],   'typ_ppp', 'typ_ppp_section_main' );
		add_settings_field( 'typ_ppp_search_count',    __( 'Search posts per page', 'ty-plugin' ),     [ $this, 'output_search_count_text' ],   'typ_ppp', 'typ_ppp_section_main' );
		add_settings_field( 'typ_ppp_default_count',   __( 'Default posts per page', 'ty-plugin' ),    [ $this, 'output_default_count_text' ],  'typ_ppp', 'typ_ppp_section_main' );

		add_settings_field( 'typ_ppp_post_type_count', '', [ $this, 'output_post_type_count_text' ], 'typ_ppp_custom', 'typ_ppp_section_custom' );
	...
	public function output_main_section_text() {
		?>
		<h2><?php _e( 'Main Settings', 'ty-plugin' ); ?></h2>
		...
		that view and will instead allow default values to pass through.', 'ty-plugin' ); ?></p>
		<p><?php _e( 'Please Note', 'ty-plugin' ); ?>:
		...
		the box on the RIGHT controls the number of posts seen on pages 2, 3, 4, etc... of that view.', 'ty-plugin' ); ?></em></p>
		<?php
	}
	...
	public function output_custom_section_text() {
		...
		?>
		<h2><?php _e( 'Custom Post Type Specific Settings', 'ty-plugin' ); ?></h2>
	...
	public function output_post_type_count_text() {
		...
		$typ_ppp_options = get_option( 'typ_ppp_options' );
		...
		$args            = apply_filters( 'typ_output_post_type_count_text', $args );
		...
		foreach ( $all_post_types as $p => $k ) {
			 ...
			 *  assign a good value. */
			if ( empty( $typ_ppp_options[ $p . '_count' ] ) ) {
				$typ_ppp_options[ $p . '_count' ] = 0;
			}

			if ( empty( $typ_ppp_options[ $p . '_count_paged' ] ) ) {
				$typ_ppp_options[ $p . '_count_paged' ] = 0;
			}
	...
	public function modify_query( $query ) {
		...
		if ( $query->is_home() ) {
			$this->process_options( 'front', $typ_ppp_options );
		} elseif ( $query->is_post_type_archive( $post_type_array ) ) {
			$current_post_type_object = $query->get_queried_object();
			$this->process_options( $current_post_type_object->name, $typ_ppp_options );
		} elseif ( $query->is_category() ) {
			$this->process_options( 'category', $typ_ppp_options );
		} elseif ( $query->is_tag() ) {
			$this->process_options( 'tag', $typ_ppp_options );
		} elseif ( $query->is_author() ) {
			$this->process_options( 'author', $typ_ppp_options );
		} elseif ( $query->is_search() ) {
			$this->process_options( 'search', $typ_ppp_options );
			...
			$this->process_options( 'archive', $typ_ppp_options );
		} else {
			$this->process_options( 'default', $typ_ppp_options );
		}
	 ...
	 * @param  $typ_ppp_options array of options from the database for custom posts per page.
	 * @return void
	 */
	public function process_options( $option_prefix, $typ_ppp_options ) {

		if ( ! $this->paged_view && ! empty( $typ_ppp_options[ $option_prefix . '_count' ] ) ) {

			$this->final_options['posts']  = $typ_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['offset'] = 0;
			$this->final_options['set_count'] = $typ_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['set_count_paged'] = $typ_ppp_options[ $option_prefix . '_count_paged' ];

		} elseif ( $this->paged_view & ! empty( $typ_ppp_options[ $option_prefix . '_count_paged' ] ) ) {

			$this->page_count_offset = ( $typ_ppp_options[ $option_prefix . '_count_paged' ] - $typ_ppp_options[ $option_prefix . '_count' ] );
			$this->final_options['offset']  = ( ( $this->page_number - 2 ) * $typ_ppp_options[ $option_prefix . '_count_paged' ] + $typ_ppp_options[ $option_prefix . '_count' ] );
			$this->final_options['posts']   = $typ_ppp_options[ $option_prefix . '_count_paged' ];
			$this->final_options['set_count'] = $typ_ppp_options[ $option_prefix . '_count' ];
			$this->final_options['set_count_paged'] = $typ_ppp_options[ $option_prefix . '_count_paged' ];
